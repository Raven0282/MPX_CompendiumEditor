    ...
    protected override async Task UpdateListingFileAsync(string repositoryPath, string id, ExtractedMetadata meta, bool isAppend)
    {
        ...
        foreach (var node in dataMatrix)
        {
            if (node is JsonArray row && row.Count > 0 && row[0]?.ToString() == id)
            {
                ...
                if (idxCategory != -1 && row.Count > idxCategory) row[idxCategory] = JsonValue.Create(meta.Tier);
                if (idxType != -1 && row.Count > idxType) row[idxType] = JsonValue.Create(meta.Prerequisite);
        ...
        if (!found && isAppend)
        {
            ...
            for (int i = 0; i < headers.Count; i++)
            {
                ...
                else if (i == idxCategory) newRow.Add(JsonValue.Create(meta.Tier));
                else if (i == idxType) newRow.Add(JsonValue.Create(meta.Prerequisite));
