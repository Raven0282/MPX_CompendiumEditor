// File: Services/Data/WeaponCompendiumWriter.cs
...
/// <summary>
/// Specialized writer for the Weapon category, handling mundane and magical weapon formats and 7-column matrices.
/// </summary>
public class WeaponCompendiumWriter : BaseCompendiumWriter
{
    public WeaponCompendiumWriter(ICompendiumExtractor extractor, IDiagnosticLogger logger) 
    ...
    {
        return repositoryPath.Contains($"{Path.DirectorySeparatorChar}weapon", StringComparison.OrdinalIgnoreCase) || 
               repositoryPath.EndsWith("weapon", StringComparison.OrdinalIgnoreCase);
    }

    protected override ExtractedMetadata ExtractMetadataFromHtml(string id, string html, string activeTargetShardPath)
    {
        // Name: <h1 class=player>Morningstar</h1> or <h1 class=mihead>Howling Songbow<span...
        ...
        {
            throw new CompendiumValidationException("Could not extract Weapon Name from <h1> tag.", activeTargetShardPath, id);
        }
        ...
        if (html.Contains("class=mihead", StringComparison.OrdinalIgnoreCase))
        {
            ...
            // Type for magic: <b>Weapon: </b>Any ranged
            var typeMatch = Regex.Match(html, @"<b>Weapon:\s*</b>(.*?)</p>", RegexOptions.IgnoreCase);
            if (typeMatch.Success) weaponType = Regex.Replace(typeMatch.Groups[1].Value, @"<[^>]+>", "").Trim();
        ...
        else // MUNDANE WEAPON OR AMMUNITION
        {
            // Type for mundane: <b>Group</b>: <br>Mace
            var groupMatch = Regex.Match(html, @"<b>Group</b>:\s*(?:<br>)?\s*(.*?)(?:\(|\.|<br>)", RegexOptions.IgnoreCase);
            if (groupMatch.Success) weaponType = Regex.Replace(groupMatch.Groups[1].Value, @"<[^>]+>", "").Trim();

            // Category check for Ammunition
            if (html.Contains("<b>Category</b>: Ammunition", StringComparison.OrdinalIgnoreCase))
            {
                weaponType = "Ammunition";
                var priceMatch = Regex.Match(html, @"<b>Price</b>:\s*(.*?)(?:<br>|$)");
                if (priceMatch.Success) cost = priceMatch.Groups[1].Value.Trim();
            }
            else
            {
                // Cost for mundane: Cost: 10 gp
                var costMatch = Regex.Match(html, @"Cost:\s*(.*?)(?:<br>|$)", RegexOptions.IgnoreCase);
        ...
        return new WeaponMetadata
        {
            ...
            Prerequisite = weaponType, // Used for Type column
    ...
    protected override async Task UpdateListingFileAsync(string repositoryPath, string id, ExtractedMetadata meta, bool isAppend)
    {
        ...
        // Weapon Matrix: ["ID", "Name", "Type", "Level", "Cost", "Rarity", "SourceBook"]
        bool found = false;
        foreach (var node in dataMatrix)
        {
            if (node is JsonArray row && row.Count >= 7 && row[0]?.ToString() == id)
            {
                row[1] = JsonValue.Create(wMeta.Name);
                row[2] = JsonValue.Create(wMeta.WeaponType);
                row[3] = JsonValue.Create(wMeta.Tier); // Level
                row[4] = JsonValue.Create(wMeta.Cost);
                row[5] = JsonValue.Create(wMeta.Rarity);
                row[6] = JsonValue.Create(wMeta.SourceBook);
                found = true;
                break;
            }
        }
        ...
        if (!found && isAppend)
        {
            _logger.Log($"Appending new weapon row to _listing.js matrix for ID: {id}", "WRITER:WEAPON");
            var newRow = new JsonArray
            {
                JsonValue.Create(id),
                JsonValue.Create(wMeta.Name),
                JsonValue.Create(wMeta.WeaponType),
                JsonValue.Create(wMeta.Tier),
                JsonValue.Create(wMeta.Cost),
                JsonValue.Create(wMeta.Rarity),
                JsonValue.Create(wMeta.SourceBook)
            };
    ...
    private class WeaponMetadata : ExtractedMetadata
    {
