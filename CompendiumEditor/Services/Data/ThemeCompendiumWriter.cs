// File: Services/Data/ThemeCompendiumWriter.cs
...
/// <summary>
/// Specialized writer for the Theme category, handling 4-column listing matrices and prerequisite extraction.
/// </summary>
public class ThemeCompendiumWriter : BaseCompendiumWriter
{
    public ThemeCompendiumWriter(ICompendiumExtractor extractor, IDiagnosticLogger logger) 
    ...
    {
        return repositoryPath.Contains($"{Path.DirectorySeparatorChar}theme", StringComparison.OrdinalIgnoreCase) || 
               repositoryPath.EndsWith("theme", StringComparison.OrdinalIgnoreCase);
    }

    protected override ExtractedMetadata ExtractMetadataFromHtml(string id, string html, string activeTargetShardPath)
    {
        // Name: <h1 class=player>Noble</h1>
        var nameMatch = Regex.Match(html, @"<h1[^>]*>(.*?)</h1>", RegexOptions.IgnoreCase);
        if (!nameMatch.Success)
        {
            throw new CompendiumValidationException("Could not extract Theme Name from <h1> tag.", activeTargetShardPath, id);
        }

        string cleanName = Regex.Replace(nameMatch.Groups[1].Value, @"<[^>]+>", "").Trim();

        // Prerequisite: <b>Prerequisite: </b>Human or halfling.<br>
        string prereq = "";
        ...
        {
            Name = cleanName,
            Tier = "", // Not used in theme listing matrix
            Prerequisite = prereq,
            BenefitText = "", // Not used in theme listing matrix
    ...
    protected override async Task UpdateListingFileAsync(string repositoryPath, string id, ExtractedMetadata meta, bool isAppend)
    {
        ...
        // Theme Matrix: ["ID", "Name", "Prerequisite", "SourceBook"]
        bool found = false;
        ...
        if (!found && isAppend)
        {
            _logger.Log($"Appending new theme row to _listing.js matrix for ID: {id}", "WRITER:THEME");
